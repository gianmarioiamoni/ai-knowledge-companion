-- Chat System Schema for AI Knowledge Companion
-- This schema supports conversations between users and AI tutors
...
-- RAG context table (stores retrieved chunks for each message)
CREATE TABLE IF NOT EXISTS message_rag_context (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_message_rag_context_message_id ON message_rag_context(message_id);
CREATE INDEX IF NOT EXISTS idx_message_rag_context_chunk_id ON message_rag_context(chunk_id);
...
-- RLS Policies for message_rag_context
...
-- Function to get conversation with tutor info
...
-- Function to get messages with RAG context
